...
class GetMajorsByNumberItemUseCase(private val registerRepository: RegisterRepository) {

    operator fun invoke(limit: Int): Flow<Resource<List<MajorRes>>> = flow {
        try {
            emit(Resource.Loading())
            val responseDto = registerRepository.getMajorsByNumberItem(limit)
