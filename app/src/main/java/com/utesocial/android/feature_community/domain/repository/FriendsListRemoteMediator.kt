...
class FriendsListRemoteMediator(
    private val friendsListDatabase: FriendsListDatabase,
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, User>): MediatorResult {
        ...
        return suspendCoroutine { continuation ->
            communityUseCase.getFriendsListUseCase
                .invoke(page, state.config.pageSize)
                .process(
                    disposable,
                    onStateChanged = object :
                        ...
                        override fun onChanged(response: SimpleResponse<AppResponse<FriendsListResponse>?>) {
                            ...
                            if (response.isSuccessful()) {
                                val endOfList = response.getResponseBody()?.data?.friends?.isEmpty()
                                this@FriendsListRemoteMediator.coroutineScope.launch {
                                    friendsListDatabase.withTransaction {
                                        if (loadType == LoadType.REFRESH) {
                                            friendsListDatabase.getRemoteKeysDao().clearAll()
                                            friendsListDatabase.getFriendsListDao().clearAll()
                                        }
                                        val prevKey =
                                        ...
                                        keys?.let {
                                            friendsListDatabase.getRemoteKeysDao().insertRemote(it)
                                        }
                                        response.getResponseBody()?.data?.friends?.let {
                                            friendsListDatabase.getFriendsListDao().insertAll(it)
                                        }
                                    }
                                    endOfList?.let {
                                        continuation.resume(MediatorResult.Success(endOfList))
                                    }
        ...
        return withContext(Dispatchers.IO) {
            state.anchorPosition?.let { position ->
                state.closestItemToPosition(position)?.userId?.let { repId ->
                    friendsListDatabase.getRemoteKeysDao().getRemoteKeys(repId)
                }
            }
        }
    }
    ...
    private suspend fun getLastRemoteKey(state: PagingState<Int, User>): FriendsListRemoteKeys? {
        return withContext(Dispatchers.IO) {
            state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
                ?.let { user -> friendsListDatabase.getRemoteKeysDao().getRemoteKeys(user.userId) }
        }
    }
