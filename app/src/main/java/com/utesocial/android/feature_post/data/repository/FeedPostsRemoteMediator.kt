...
class FeedPostsRemoteMediator(
    ...
    override suspend fun load(
        ...
        return suspendCoroutine { continuation ->
            postUseCase.getFeedPostsUseCase
                .invoke(page, state.config.pageSize, Like.UserType.USER)
                .process(
                    disposable,
                    onStateChanged = object :
                        ...
                        override fun onChanged(response: SimpleResponse<AppResponse<PostBody>?>) {
                            if(response.isSuccessful()) {
                                ...
                                this@FeedPostsRemoteMediator.coroutineScope.launch {
                                    postDatabase.withTransaction {
                                        if(loadType == LoadType.REFRESH) {
                                            postDatabase.getPostRemoteKeysDao().clearAll()
                                        ...
                                        val keys =
                                            response.getResponseBody()?.data?.posts?.map { post ->
                                                PostRemoteKeys(post.id, prevKey, nextKey)
                                            }
                                        keys?.let {
                                            postDatabase.getPostRemoteKeysDao().insertRemote(keys)
                                        }
    ...
    private suspend fun getRefreshRemoteKey(state: PagingState<Int, PostModel>): PostRemoteKeys? {
        return withContext(Dispatchers.IO) {
            state.anchorPosition?.let { position ->
                state.closestItemToPosition(position)?.id?.let { repId ->
                    postDatabase.getPostRemoteKeysDao().getRemoteKeys(repId)
                }
            }
        }
    }

    private suspend fun getLastRemoteKey(state: PagingState<Int, PostModel>): PostRemoteKeys? {
        return withContext(Dispatchers.IO) {
            state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
                ?.let { post -> postDatabase.getPostRemoteKeysDao().getRemoteKeys(post.id) }
        }
    }

    private suspend fun getFirstRemoteKey(state: PagingState<Int, PostModel>): PostRemoteKeys? {
        return withContext(Dispatchers.IO) {
